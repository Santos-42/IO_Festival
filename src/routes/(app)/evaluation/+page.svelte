  ...
  async function startEvaluation() {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        throw new Error(
          errData.error || errData.details || "Gagal mengambil soal dari AI",
        );
      }
...
<div class="space-y-8 pb-20">
  ...
  {#if data.activeRole && !data.roadmapCompleted}
    ...
    <div
      class="bg-white rounded-[2.5rem] p-10 border border-gray-100 shadow-xl shadow-amber-50/50 flex flex-col items-center text-center space-y-6"
      in:fade
    >
      <div
        class="w-20 h-20 bg-amber-50 rounded-full flex items-center justify-center"
      >
      ...
      <div class="space-y-3">
        ...
        <p class="text-gray-500 max-w-md leading-relaxed">
          Evaluasi hanya bisa diakses setelah kamu menyelesaikan seluruh materi
          dan kuis di roadmap
          <span class="text-blue-600 font-bold"
            >{data.activeRole.role_name}</span
          >
          hingga <strong>100%</strong>.
        </p>
      </div>
      ...
      {#if data.progress}
        <div class="w-full max-w-md space-y-3">
          <div
            class="flex justify-between text-xs text-gray-400 font-bold uppercase tracking-wider"
          >
          ...
          <div class="flex justify-between text-xs text-gray-400">
            <span
              >{data.progress.completedCheckpoints +
                data.progress.completedQuizzes} dari {data.progress
                .totalMaterials + data.progress.totalQuizzes} selesai</span
            >
            <span class="text-blue-600 font-bold"
              >{data.progress.totalMaterials +
                data.progress.totalQuizzes -
                data.progress.completedCheckpoints -
                data.progress.completedQuizzes} tersisa</span
            >
          </div>
        </div>
      {/if}
    ...
    {#if data.pastEvaluations.length > 0}
      <div class="space-y-4 mt-8">
        <h4 class="text-sm font-black text-gray-800 ml-2">
          Riwayat Evaluasi Sebelumnya
        </h4>
        <div class="grid grid-cols-1 gap-4">
          {#each data.pastEvaluations as eval_item}
            <a
              href="/evaluation/review/{eval_item.id}"
              class="bg-white p-6 rounded-[2rem] border border-gray-100 shadow-sm flex items-center justify-between group hover:border-blue-200 transition-all"
            >
              <div class="flex items-center space-x-4">
                ...
                <div>
                  ...
                  >
                    {new Date(eval_item.created_at).toLocaleDateString()} • {eval_item
                      .feedback_json.decision}
                  </p>
                </div>
              </div>
              <span
                class="text-blue-600 text-xs font-bold hover:underline shrink-0"
                >Lihat Detail →</span
              >
            </a>
          {/each}
  ...
  {:else if currentState === "idle"}
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-8">
      <div class="md:col-span-2 space-y-6">
        <div
          class="bg-white rounded-[2.5rem] p-8 border border-gray-100 shadow-xl shadow-blue-50/50 relative overflow-hidden"
          in:fade
        >
          <div
            class="absolute -top-10 -right-10 w-40 h-40 bg-blue-50 rounded-full blur-3xl opacity-50"
          ></div>

          <div class="relative space-y-6">
            <div
              class="w-16 h-16 bg-blue-600 rounded-2xl flex items-center justify-center shadow-lg shadow-blue-200"
            >
              <ClipboardList class="text-white" size={32} />
            </div>

            <div>
              <h3 class="text-base font-black text-gray-800 mb-2">
                Siap untuk diuji?
              </h3>
              <p class="text-gray-500 text-sm leading-relaxed">
                Kami akan menyiapkan sebuah <strong>Mini Case Study</strong>
                yang dirancang khusus untuk posisi
                <span class="text-blue-600 font-bold"
                  >{data.activeRole?.role_name || "???"}</span
                >
            ...
            {#if data.activeRole}
              {#if evaluationCooldown > 0}
                <div
                  class="w-full bg-amber-50 border border-amber-200 rounded-[1.5rem] p-4 text-center"
                >
                  <p class="text-amber-700 font-bold">
                    Cooldown: {formatTime(evaluationCooldown)}
                  </p>
                  <p class="text-amber-600 text-sm mt-1">
                    Silakan pelajari ulang materi sebelum mencoba lagi
                  </p>
                </div>
              {:else}
                <button
            ...
            {:else}
              <div
                class="p-4 bg-red-50 border border-red-100 rounded-2xl flex items-center space-x-3 text-red-600"
              >
                <AlertCircle size={20} />
                <p class="text-sm font-bold">
                  Pilih roadmap aktif terlebih dahulu di halaman Roadmap.
                </p>
              </div>
            {/if}
        ...
        <div class="space-y-4">
          ...
          {#if data.pastEvaluations.length === 0}
            <div
              class="bg-gray-50 rounded-[2rem] p-10 text-center border border-dashed border-gray-200"
            >
      ...
      <div class="md:col-span-1">
        <div
          class="bg-gradient-to-br from-blue-600 to-blue-800 rounded-[2.5rem] p-8 text-white shadow-xl shadow-blue-200 sticky top-8"
        >
  ...
  {:else if currentState === "loading"}
    ...
    >
      <div class="relative">
        <div
          class="w-24 h-24 border-4 border-blue-100 border-t-blue-600 rounded-full animate-spin"
        ></div>
        <div
          class="absolute inset-0 flex items-center justify-center text-blue-600"
        >
      ...
      <div class="text-center">
        ...
        <p class="text-gray-400 font-medium">
          AI sedang menyusun tantangan untuk Anda...
        </p>
      </div>
    </div>
  {:else if currentState === "active"}
    ...
    <div class="max-w-4xl mx-auto space-y-8" in:fade>
      <!-- Case Study Card -->
      <div
        class="bg-blue-600 rounded-[2.5rem] p-8 text-white shadow-2xl shadow-blue-200 relative overflow-hidden"
      >
        ...
        <div class="relative space-y-4">
          <div
            class="flex items-center space-x-2 text-blue-100 text-[10px] font-black uppercase tracking-widest"
          >
            <ClipboardList size={14} />
            <span>Mini Case Study</span>
          </div>
          <h3 class="text-base font-black leading-relaxed whitespace-pre-wrap">
      ...
      <!-- Question & Answer -->
      <div
        class="bg-white rounded-[2.5rem] p-8 border border-gray-100 shadow-xl shadow-blue-50/50 space-y-6"
      >
        <div>
          <label
            for="answer"
          ...
          >
          <p
            class="text-sm font-bold text-gray-800 leading-relaxed whitespace-pre-wrap"
          >
        ...
        <div class="space-y-2">
          ...
          <div class="flex justify-end">
            <span class="text-xs font-bold text-gray-400"
              >{userAnswer.length} karakter</span
            >
          </div>
        </div>
  ...
  {:else if currentState === "evaluating"}
    ...
    >
      <div class="relative">
        <div
          class="w-24 h-24 border-4 border-gray-100 border-t-green-500 rounded-full animate-spin"
        ></div>
        <div
      ...
      <div class="text-center">
        ...
        <p class="text-gray-400 font-medium">
          AI sedang menganalisis solusi Anda...
        </p>
      </div>
    </div>
  {:else if currentState === "result"}
    ...
    <div class="max-w-4xl mx-auto space-y-8" in:fade>
      <div
        class="bg-white rounded-[2.5rem] p-10 border border-gray-100 shadow-2xl shadow-blue-50/50 flex flex-col items-center text-center space-y-6"
      >
        <div class="relative">
          <div
            class="w-24 h-24 rounded-full border-4 border-gray-50 flex items-center justify-center"
          >
          ...
          <div
            class="absolute -bottom-2 left-1/2 -translate-x-1/2 px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-widest shadow-md
        ...
        <div class="space-y-2">
          ...
          <p class="text-sm text-gray-500 italic max-w-lg">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4 w-full">
          <div
            class="bg-green-50 p-6 rounded-[2rem] border border-green-100 text-left"
          >
          ...
          <div
            class="bg-red-50 p-6 rounded-[2rem] border border-red-100 text-left"
          >
