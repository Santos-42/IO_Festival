...
<script lang="ts">
  ...
  function cleanContent(text: string) {
    ...
    cleaned = cleaned
      ...
      .replace(
        /([a-zA-Z0-9>]{2,}|[a-z]|>)\.\s+([A-Z][a-zA-Z0-9\s\&\(\)-]{1,40}:)\s/g,
        "$1.\n\n$2 ",
      )
  ...
  const prevMaterial = $derived(
    data.currentIndex > 0 ? data.allMaterials[data.currentIndex - 1] : null,
  );
  const nextMaterial = $derived(
    data.currentIndex < data.totalMaterials - 1
      ? data.allMaterials[data.currentIndex + 1]
      : null,
  );
...
<div class="max-w-5xl mx-auto space-y-8 pb-20">
  ...
  <div class="flex items-center justify-between">
    ...
    <div
      class="px-4 py-1.5 bg-blue-50 text-blue-600 rounded-full text-xs font-black uppercase tracking-wider"
    >
      {data.moduleName}
    </div>
  </div>
  ...
  <!-- Content Card -->
  <div
    class="bg-white rounded-[2.5rem] p-8 border border-gray-100 shadow-xl shadow-blue-50/50 space-y-8"
    in:fade
  >
    ...
    {#if videoId}
      <div
        class="aspect-video w-full rounded-[2rem] overflow-hidden shadow-2xl bg-black border-4 border-white"
      >
        <iframe
          width="100%"
          height="100%"
    ...
    {:else}
      <div
        class="aspect-video w-full rounded-[2rem] bg-gray-100 flex flex-col items-center justify-center space-y-4 border-2 border-dashed border-gray-200"
      >
        <div
          class="w-16 h-16 bg-white rounded-2xl flex items-center justify-center shadow-sm text-gray-300"
        >
          <Play size={32} />
        </div>
  ...
  <div class="space-y-6 pt-4">
    <!-- Checkpoint Gate (always visible, shows success state when passed) -->
    <CheckpointGate
      materialId={data.material.id}
      onPass={() => (checkpointPassed = true)}
      alreadyPassed={data.checkpointAlreadyPassed}
    />

    <!-- Quiz Button (shown after last material, only when checkpoint passed) -->
    {#if isLastInModule && !nextMaterial && checkpointPassed}
      <div
        class="bg-white rounded-[2.5rem] p-6 border-2 border-blue-100 shadow-lg shadow-blue-50/50"
        in:fade
      >
        <div class="flex items-center justify-between">
          <div class="flex items-center space-x-4">
            ...
            <div>
              ...
              <p class="text-sm text-gray-500">
                Uji pemahaman Anda untuk modul ini
              </p>
            </div>
          </div>
          <a
    ...
    <div class="flex items-center justify-between gap-4">
      ...
      {#if nextMaterial}
        {#if isLastInModule && checkpointPassed}
          <a
            href="/roadmap/{data.moduleId}/quiz"
            class="flex-1 flex items-center justify-center space-x-3 px-6 py-5 rounded-[2rem] font-black transition-all group bg-green-600 hover:bg-green-700 text-white shadow-xl shadow-green-200"
          >
            <div class="text-right">
              <p class="text-[10px] uppercase tracking-widest opacity-70">
                Selanjutnya
              </p>
              <p class="truncate max-w-[200px]">Quiz {data.moduleName}</p>
            </div>
            <ArrowRight
              size={20}
              class="group-hover:translate-x-1 transition-transform"
        ...
        {/if}
      {:else if isLastInModule && checkpointPassed}
        <a
          href="/roadmap/{data.moduleId}/quiz"
          class="flex-1 flex items-center justify-center space-x-3 px-6 py-5 rounded-[2rem] font-black transition-all group bg-green-600 hover:bg-green-700 text-white shadow-xl shadow-green-200"
        >
